...
- Meios de pagamento locais e globais:
  ...
  * Cartão de Crédito/Débito (Visa, Mastercard) para compradores internacionais pagando em Dólares, Euros ou Reais.
...
// Helper for fallback intelligent responses if API key is not yet configured
...
5. **100% - Escala & Levantamentos**: Acompanhe as vendas no *Painel*, veja os lucros a entrar na *Carteira* e faça levantamento para a sua conta bancária angolana (taxa fixa de 500 Kz).
...
- 💳 **Cartão Visa/Mastercard (Internacional)**: Aceita pagamentos em **Dólar ($)**, **Euro (€)** e **Real (R$)** para compradores em Portugal, Brasil, EUA ou qualquer país do mundo.
...
- **Levantamento Bancário em Angola**:
  - Taxa fixa de apenas **500 Kz** por transferência bancária (independentemente do valor levantado).
  ...
  - Liquidação rápida diretamente na sua conta bancária.`;
  }
...
- **Levantamentos**: Saiba como receber os lucros diretamente na sua conta bancária em Angola.
...
app.post("/api/gemini/chat", async (req, res) => {
  const { message, history = [], userRole = "creator" } = req.body;
  ...
  if (!ai) {
    // If no API key is supplied yet in environment, return rich smart fallback immediately
...
Responda de forma completa, prática e estruturada sobre como funciona a Comerça ou como usar a funcionalidade solicitada.`;
    ...
    const generatePromise = ai.models.generateContent({
      ...
      config: {
        systemInstruction: COMERCA_KNOWLEDGE,
        temperature: 0.7,
      },
    });
...
app.get("/api/health", (req, res) => {
  res.json({ status: "ok", service: "comerca-api" });
});

// ---------------------------------------------------------------------
// Email confirmation codes
//
// Codes are signed with an HMAC token instead of kept in server memory,
// so the same logic works identically whether this runs as a long-lived
// Express process or as stateless Netlify Functions (see
// netlify/functions/send-code.js and verify-code.js). The raw code is
// NEVER sent back in the API response — it is only delivered via the
// actual email (through Resend, https://resend.com). If RESEND_API_KEY
// is not configured, the code is logged to the server console instead,
// so the flow can still be tested locally without a real inbox, but it
...
function signCode(email: string, code: string, expiresAt: number, secret: string): string {
  return crypto
...
async function sendCodeByEmail(toEmail: string, name: string, code: string): Promise<boolean> {
  const apiKey = process.env.RESEND_API_KEY;
  const fromAddress = process.env.RESEND_FROM_EMAIL || "Comerça <onboarding@resend.dev>";

  if (!apiKey || apiKey === "MY_RESEND_API_KEY") {
    // No email provider configured yet: log server-side only so the
    // developer can still test locally, but never send this to the client.
...
// Generate a code and email it to the given address
app.post("/api/auth/send-code", async (req, res) => {
  ...
  if (!secret || secret === "MY_CODE_SIGNING_SECRET") {
    res.status(500).json({
      success: false,
      error: "CODE_SIGNING_SECRET não está configurado em .env.",
    });
    return;
  }
  ...
  const token = signCode(email, code, expiresAt, secret);
  ...
  res.json({
    ...
    expiresAt,
    token,
    // Only ever a human-readable notice, never the code itself.
    message: delivered
      ? "Código enviado para o seu email."
      : "Serviço de email ainda não está configurado neste servidor (ver RESEND_API_KEY em .env). O código foi gerado e registado nos logs do servidor para testes.",
  });
});

// Verify a submitted code against its signed token
app.post("/api/auth/verify-code", (req, res) => {
  const { email, code, token, expiresAt } = req.body;

  if (!email || !code || !token || !expiresAt) {
    res.json({ valid: false, error: "Dados em falta. Peça um novo código." });
    return;
  }
  ...
  if (Date.now() > Number(expiresAt)) {
    res.json({ valid: false, error: "Código expirado. Peça um novo código." });
    return;
  }

  const expectedToken = signCode(email, String(code).trim(), Number(expiresAt), secret);
  ...
  if (!valid) {
    res.json({ valid: false, error: "Código incorreto." });
    return;
  }
